...
partial_new(PyTypeObject *type, PyObject *args, PyObject *kw)
{
    ...
    /* Owned reference when merging a nested partial; its fields may be
       replaced concurrently by __setstate__ on the free-threaded build. */
    PyObject *merged_fn = NULL;
                   ...
                   size_t nargsf, PyObject *kwnames)
{
    ...
    /* Take strong references to the mutable fields under the object lock:
       __setstate__ may replace them concurrently. */
...
partial_repr(PyObject *self)
{
    ...
    /* Reference arguments in case they change; take them under the object
       lock since __setstate__ may replace them concurrently. */
    PyObject *fn, *args, *kw;
    Py_BEGIN_CRITICAL_SECTION(pto);
    fn = Py_NewRef(pto->fn);
...
partial_reduce(PyObject *self, PyObject *Py_UNUSED(args))
{
    ...
    /* Take strong references to the mutable fields under the object lock:
       __setstate__ may replace them concurrently. */
    PyObject *fn, *fargs, *fkw, *fdict;
    Py_BEGIN_CRITICAL_SECTION(pto);
    fn = Py_NewRef(pto->fn);
    fargs = Py_NewRef(pto->args);
    fkw = Py_NewRef(pto->kw);
    fdict = Py_XNewRef(pto->dict);
    Py_END_CRITICAL_SECTION();
    PyObject *result = Py_BuildValue("O(O)(OOOO)", Py_TYPE(pto), fn, fn,
                                     fargs, fkw, fdict ? fdict : Py_None);
    Py_DECREF(fn);
    Py_DECREF(fargs);
    Py_DECREF(fkw);
    Py_XDECREF(fdict);
...
partial_setstate(PyObject *self, PyObject *state)
{
    ...
    /* Swap the fields under the object lock so that concurrent readers
       always see a consistent snapshot; release the old references only
       after the lock is dropped. */
    PyObject *new_fn = Py_NewRef(fn);
    ...
    old_kw = pto->kw;
    old_dict = pto->dict;
    FT_ATOMIC_STORE_PTR_RELEASE(pto->fn, new_fn);
    FT_ATOMIC_STORE_PTR_RELEASE(pto->args, fnargs);
    FT_ATOMIC_STORE_PTR_RELEASE(pto->kw, kw);
    pto->phcount = phcount;
    FT_ATOMIC_STORE_PTR_RELEASE(pto->dict, dict);
    partial_setvectorcall(pto);
    Py_END_CRITICAL_SECTION();
    Py_DECREF(old_fn);
    Py_DECREF(old_args);
